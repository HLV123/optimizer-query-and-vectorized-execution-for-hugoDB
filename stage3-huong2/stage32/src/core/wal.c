...
int64_t wal_log_mvcc_begin(Wal *w, uint64_t tx_id, uint64_t begin_ts) {
    /* Encode begin_ts vào page_id field (8 bytes), length=0 */
    return wal_append(w, WAL_MVCC_BEGIN, tx_id, begin_ts, 0, NULL, NULL, 0);
}

int64_t wal_log_mvcc_commit(Wal *w, uint64_t tx_id, uint64_t commit_ts) {
    return wal_append(w, WAL_MVCC_COMMIT, tx_id, commit_ts, 0, NULL, NULL, 0);
}

int64_t wal_log_mvcc_version(Wal *w, uint64_t tx_id, uint64_t doc_id,
                              uint64_t version_ptr, const char *coll_name) {
    /* Encode vào before[]: doc_id(8) + version_ptr(8) + coll_name(up to N bytes) */
    uint8_t payload[128];
    memset(payload, 0, sizeof(payload));
    write_u64_be(payload,     doc_id);
    write_u64_be(payload + 8, version_ptr);
    size_t clen = coll_name ? strlen(coll_name) : 0;
    if (clen > 64) clen = 64;
    if (coll_name) memcpy(payload + 16, coll_name, clen);
    uint16_t plen = (uint16_t)(16 + clen);
    /* before = payload, after = NULL (same bytes for simplicity) */
    return wal_append(w, WAL_MVCC_VERSION, tx_id, 0, 0, payload, payload, plen);
}
