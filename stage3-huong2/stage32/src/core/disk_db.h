...
#define PAGE_TYPE_MVCC_VERSION 0x08 /* page chứa DocVersion (MVCC mode) */

/* ===== Isolation mode ===== */
typedef enum {
    HUGO_MODE_2PL  = 0,   /* Two-Phase Locking (default, behavior như stage 1-2) */
    HUGO_MODE_MVCC = 1,   /* Multi-Version Concurrency Control (stage 3+) */
} IsolationMode;
...
typedef struct {
    ...
    /* ===== MVCC fields (Stage 3) ===== */
    IsolationMode     mode;               /* HUGO_MODE_2PL hoặc HUGO_MODE_MVCC */
    TsOracle          mvcc_oracle;        /* monotonic timestamp counter */
    MvccTxRegistry    mvcc_registry;      /* active MVCC transactions */
    MvccCommittedTable mvcc_committed;    /* recently committed/aborted tx records */
} DiskDB;

/* Lifecycle */
int  ddb_create(DiskDB *db, const char *name, const char *path);
int  ddb_open  (DiskDB *db, const char *path);
/* Open với mode (MVCC hoặc 2PL) */
