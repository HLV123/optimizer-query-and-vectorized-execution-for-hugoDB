/* disk_db.c â€” Disk-backed database implementation */
#include "disk_db.h"
...
/* Derive log path: "xxx.hugo" â†’ "xxx.hugolog" */
static void derive_log_path(const char *db_path, char *log_path, size_t max) {
    size_t n = strlen(db_path);
    /* Náº¿u káº¿t thÃºc ".hugo" thÃ¬ thay báº±ng ".hugolog"; náº¿u khÃ´ng append ".hugolog" */
...
/* Write document vÃ o buffer (fit trong page data area 4077 bytes).
 * Tráº£ vá» sá»‘ bytes Ä‘Ã£ ghi, -1 náº¿u overflow. */
...
static int save_meta(DiskDB *db) {
    ...
    /* Allocate META page náº¿u chÆ°a cÃ³ */
    uint64_t meta_pid = 1;
...
int ddb_open(DiskDB *db, const char *path) {
    ...
    /* Open WAL vÃ  cháº¡y recovery náº¿u cÃ³ record */
    derive_log_path(path, db->log_path, sizeof(db->log_path));
    if (wal_open(&db->wal, db->log_path) == WAL_OK) {
        db->wal_enabled = 1;
        /* Recovery: REDO + UNDO náº¿u log cÃ³ record */
        if (db->wal.size > 0) {
            printf("(found WAL, running recovery...)\n");
            wal_recover(&db->wal, &db->pm);
            /* Clear log sau khi recovery thÃ nh cÃ´ng */
            wal_truncate(&db->wal);
        }
    }
...
int ddb_close(DiskDB *db) {
    ...
    /* Clean close â†’ truncate WAL (má»i thá»© Ä‘Ã£ Ä‘Æ°á»£c persist qua meta) */
    if (db->wal_enabled) {
        wal_truncate(&db->wal);
        wal_close(&db->wal);
        db->wal_enabled = 0;
...
int ddb_drop_coll(DiskDB *db, const char *name) {
    for (int i = 0; i < db->n_colls; i++) {
        if (strcmp(db->colls[i].name, name) == 0) {
            /* Free in-RAM index (data pages orphan â€” free list sáº½ xá»­ lÃ½) */
            free(db->colls[i].doc_page_ids);
...
int ddb_insert_doc(DiskDB *db, DiskColl *c, Document *doc, uint64_t *out_id) {
    ...
    /* Auto-add "id" field náº¿u chÆ°a cÃ³ */
    ...
    if (db->wal_enabled) {
        /* Page má»›i alloc â†’ before_image = toÃ n 0 */
...
Document* ddb_read_doc(DiskDB *db, DiskColl *c, uint64_t id) {
    ...
    /* Deserialize â€” dÃ¹ng full data area (extra bytes ignored via n_pairs count) */
...
int ddb_delete_doc(DiskDB *db, DiskColl *c, uint64_t id) {
    ...
    /* Mark page free (MVP: chá»‰ zero index, khÃ´ng reuse page) */
    c->doc_page_ids[id] = 0;
    c->count--;
    db->dirty = 1;
    return 0;
...
int ddb_scan(DiskDB *db, DiskColl *c, ddb_visit_fn fn, void *ctx) {
    ...
    for (uint64_t id = 1; id < c->capacity && id < c->next_id; id++) {
        ...
        if (!d) continue;
        fn(id, d, ctx);
        /* fn lÃ  owner cá»§a d â€” náº¿u cáº§n keep, clone */
        doc_free(d);
        visited++;
    }
    return visited;
