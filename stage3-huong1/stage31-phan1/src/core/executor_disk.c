...
/* Global optimizer context — initialized once per DiskDB open.
...
int hugo_execute_disk_opt(DiskDB *db, const Query *q, HugoResult *r) {
    ...
    if (q->verb == VERB_ANALYZE) {
        ...
        if (rc != 0) {
            ...
            snprintf(r->info, sizeof(r->info),
                     "analyzed collection '%s' — statistics updated", q->collection);
            r->ok    = 1;
            r->count = 0;
        }
        return 0;
    ...
    /* If optimizer not ready or mode is OFF → legacy path */
    ...
    /* READ queries (FUNDEN, GOMAIL) → optimizer path */
    ...
    /* All other verbs (writes, DDL, transactions) → legacy path */
    return hugo_execute_disk(db, q, r);
}
