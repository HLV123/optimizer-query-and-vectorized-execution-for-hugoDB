/* optimizer.c — Hugo DB query optimizer (Phase 1-6)
 *
 * Implements cost-based selection of physical plans from logical plan.
 * Phase 7 (join order DP) is stubbed with a greedy fallback.
 */
#include "optimizer.h"
#include <stdio.h>
#include <string.h>
...
PhysicalPlan* optimizer_run(OptimizerCtx *ctx, const Query *q, Arena *arena) {
    ...
    /* Step 2: Apply logical rewrite rules */
    ...
    /* Step 3: Enumerate physical plans and select best */
    PhysicalPlan *pplan = enumerate_physical(ctx, arena, lplan);
