...
int hugo_parse(const TokenList *tl, Query *q) {
    ...
    if (q->verb == VERB_FUNDEN && !at_end(p) && peek(p)->type == TOK_OP_RASOAT) {
         ...
         * A) $rasoat <target_coll> local_field <lf> target_field <tf>
         * B) $rasoat <alias> từ <target_coll> on <lf> $bg <tf>
         * Detect by checking if next token after first IDENT is "local_field" */
        const char *first_ident = NULL;
        if (peek(p)->type == TOK_IDENT) {
            first_ident = peek(p)->text;
            advance(p);
        }
        /* Check next token to determine syntax */
        if (peek(p)->type == TOK_IDENT &&
            strcmp(peek(p)->text, "local_field") == 0) {
            /* Syntax A: first_ident = target_coll */
            if (first_ident)
                strncpy(q->join->target_coll, first_ident,
                        sizeof(q->join->target_coll)-1);
            advance(p);  /* consume "local_field" */
            if (peek(p)->type == TOK_IDENT) {
                strncpy(q->join->local_field, peek(p)->text,
                        sizeof(q->join->local_field)-1);
                advance(p);
            }
            /* consume "target_field" keyword */
            if (peek(p)->type == TOK_IDENT &&
                strcmp(peek(p)->text, "target_field") == 0)
                advance(p);
            if (peek(p)->type == TOK_IDENT) {
                strncpy(q->join->target_field, peek(p)->text,
                        sizeof(q->join->target_field)-1);
                advance(p);
            }
        } else {
            /* Syntax B: first_ident = alias */
            if (first_ident)
                strncpy(q->join->alias, first_ident,
                        sizeof(q->join->alias)-1);
            if (peek(p)->type == TOK_TU) advance(p);  /* consume 'từ'/'tu' */
            if (peek(p)->type == TOK_IDENT) {
                strncpy(q->join->target_coll, peek(p)->text,
                        sizeof(q->join->target_coll)-1);
                advance(p);
            }
