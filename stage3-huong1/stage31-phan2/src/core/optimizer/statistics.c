...
int stats_analyze(DiskDB *db, StatsStore *ss, const char *collection) {
    ...
    /* Heap-allocate: AnalyzeCtx is ~2MB, exceeds default Windows 1MB stack */
    AnalyzeCtx *ctxp = (AnalyzeCtx*)calloc(1, sizeof(AnalyzeCtx));
    if (!ctxp) return -1;
    AnalyzeCtx ctx_copy; /* local alias trick â€” use pointer throughout */
    (void)ctx_copy;

    ddb_scan(db, c, analyze_visit, ctxp);
    /* Copy to local for rest of function */
    AnalyzeCtx *ctx = ctxp;
