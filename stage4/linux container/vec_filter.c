...
void vec_filter_apply(ColBatch *b, const Condition *cond)
{
    ...
    case COND_OR: {
        /* OR: cần temp copy của alive, merge bằng OR
         * Tránh alloc: dùng stack array nếu n_rows nhỏ, heap nếu lớn */
        uint8_t *saved = NULL;
        int n = b->n_rows;
        if (n <= 65536) {
            saved = (uint8_t*)__builtin_alloca(n); /* stack, Linux/GCC/Clang ok */
        } else {
            saved = (uint8_t*)malloc(n);
        }
        if (!saved) {
            /* fallback: apply left only */
            vec_filter_apply(b, cond->left);
            break;
        }
        ...
        /* Apply left */
        vec_filter_apply(b, cond->left);
        uint8_t *left_result = (uint8_t*)__builtin_alloca(n <= 65536 ? n : 1);
        if (n <= 65536) {
            memcpy(left_result, b->alive, n);
        } else {
            /* For large n, use a different approach */
            memcpy(b->alive, saved, n); /* restore */
            vec_filter_apply(b, cond->left);
            /* Store left, then OR with right below */
            uint8_t *tmp = (uint8_t*)malloc(n);
            if (tmp) {
                memcpy(tmp, b->alive, n);
                memcpy(b->alive, saved, n);
                vec_filter_apply(b, cond->right);
                for (int i = 0; i < n; i++)
                    b->alive[i] = tmp[i] | b->alive[i];
                free(tmp);
            }
            if (n > 65536) free(saved);
            return;
        }
