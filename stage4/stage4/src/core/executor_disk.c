...
int hugo_execute_disk(DiskDB *db, const Query *q, HugoResult *r) {
    ...
    case VERB_VIETINFO: exec_vietinfo(db, q, r);
        if (r->ok) scan_cache_invalidate(vec_get_scan_cache(), q->collection);
        break;
    case VERB_COCHIN:   exec_cochin(db, q, r);
        if (r->ok) scan_cache_invalidate(vec_get_scan_cache(), q->collection);
        break;
    case VERB_DEMLET:   exec_demlet(db, q, r);
        if (r->ok) scan_cache_invalidate(vec_get_scan_cache(), q->collection);
        break;
...
int hugo_execute_disk_opt(DiskDB *db, const Query *q, HugoResult *r) {
    ...
    if (q->verb == VERB_FUNDEN || q->verb == VERB_GOMAIL) {
        ...
        int rc = vec_exec_run(db, plan, r, arena);
        arena_free(arena);
        return rc;
    }
