/* test_optimizer.c — Tests for Stage 3: Cost-Based Query Optimizer
 *
 * Tests cover:
...
#include "../src/core/optimizer/join_order.h"
#include "../src/core/optimizer/advanced_rules.h"
...
static void test_lplan_sort_limit(void) {
    TEST("lplan: scan → filter → sort → limit");
    Arena *a = arena_new();
...
static void test_lplan_join(void) {
    ...
    LogicalPlan *p = build_logical_plan(&q, a);
    ASSERT(p != NULL, "plan is null");
    /* root may be Scan or Join depending on structure — find JOIN */
...
static void test_stats_analyze_basic(void) {
    TEST("stats: analyze collection, total_rows correct");
    db_setup();
    /* Insert 20 docs */
    for (int i = 0; i < 20; i++) {
    ...
    ASSERT(ctx != NULL, "optimizer ctx is null");
    int rc = optimizer_analyze(ctx, "testcol");
...
static void test_phys_index_scan_chosen(void) {
    ...
    PhysicalPlan *cur = p;
    while (cur && cur->left) cur = cur->left;
    /* Index scan should be chosen — or at minimum the plan exists */
...
/* ===== Phase 7: Join Order DP Tests ===== */

static void test_join_order_single_table(void) {
    TEST("join_order: single table returns access path");
    db_setup();
    for (int i = 0; i < 20; i++) {
        const char *pairs[1][2] = {{"x", "1"}};
        insert_doc("jt1", pairs, 1);
    }
    OptimizerCtx *ctx = hugo_optimizer_get();
    optimizer_analyze(ctx, "jt1");
    Arena *a = arena_new();

    JoinOrderCtx jctx;
    join_order_ctx_init(&jctx, &g_db, &ctx->stats, &ctx->cost_model, a, 0);
    join_order_add_table(&jctx, "jt1", NULL);

    PhysicalPlan *p = join_order_optimize(&jctx);
    ASSERT(p != NULL, "no plan returned");
    ASSERT(p->type == POP_SEQ_SCAN || p->type == POP_FILTER ||
           p->type == POP_INDEX_SCAN, "expected scan-type plan");
...
static void test_join_order_two_tables(void) {
    TEST("join_order: two tables joined, cheaper side first");
    db_setup();
    /* Insert: 'big' collection = 100 rows, 'small' = 5 rows */
    for (int i = 0; i < 100; i++) {
        const char *pairs[1][2] = {{"id", ""}};
        char v[16]; snprintf(v, sizeof(v), "%d", i);
        pairs[0][1] = v;
        insert_doc("bigtab", pairs, 1);
    }
    for (int i = 0; i < 5; i++) {
        const char *pairs[1][2] = {{"ref_id", ""}};
        char v[16]; snprintf(v, sizeof(v), "%d", i);
        pairs[0][1] = v;
        insert_doc("smalltab", pairs, 1);
    }
    OptimizerCtx *ctx = hugo_optimizer_get();
    optimizer_analyze(ctx, "bigtab");
    optimizer_analyze(ctx, "smalltab");

    Arena *a = arena_new();
    JoinOrderCtx jctx;
    join_order_ctx_init(&jctx, &g_db, &ctx->stats, &ctx->cost_model, a, 0);
    join_order_add_table(&jctx, "bigtab", NULL);
    join_order_add_table(&jctx, "smalltab", NULL);
    join_order_add_predicate(&jctx, "bigtab", "id", "smalltab", "ref_id");

    PhysicalPlan *p = join_order_optimize(&jctx);
    ASSERT(p != NULL, "no join plan returned");
    /* Should produce a join plan (NL, Hash, or SMerge) */
    ASSERT(p->type == POP_NESTED_LOOP_JOIN ||
           p->type == POP_HASH_JOIN ||
           p->type == POP_SORT_MERGE_JOIN, "expected join plan");
    ASSERT(p->estimated_cost > 0, "cost should be positive");
...
static void count_split_cb(TableSet s1, TableSet s2, void *ctx_) {
    (void)s1; (void)s2; (void)ctx_;
    g_split_count++;
}

static void test_join_order_enumerate_splits(void) {
    TEST("join_order: enumerate_splits + popcount correctness");
    ASSERT(tableset_popcount(0xF) == 4, "popcount(0xF) should be 4");
    ASSERT(tableset_popcount(0xA) == 2, "popcount(0xA) should be 2");
    ASSERT(tableset_popcount(0) == 0, "popcount(0) should be 0");
    ASSERT(tableset_popcount(1) == 1, "popcount(1) should be 1");
    /* 3-table set: pairs (001,110),(010,101),(011,100) = 3 */
    g_split_count = 0;
    enumerate_splits(0x7, count_split_cb, NULL);
    ASSERT(g_split_count == 3, "expected 3 splits for 3-table set");
    /* 4-table set: 7 pairs where s1<s2 */
    g_split_count = 0;
    enumerate_splits(0xF, count_split_cb, NULL);
    ASSERT(g_split_count == 7, "expected 7 splits for 4-table set");
    PASS();
}

static void test_join_order_e2e_same_result(void) {
    TEST("join_order: e2e JOIN with DP optimizer same result as legacy");
    db_setup();
    /* Insert users and orders */
    for (int i = 1; i <= 10; i++) {
        const char *pairs[2][2] = {{"uid", ""}, {"name", "user"}};
        char v[16]; snprintf(v, sizeof(v), "%d", i);
        pairs[0][1] = v;
        insert_doc("ujusers", pairs, 2);
    }
    for (int i = 1; i <= 5; i++) {
        const char *pairs[2][2] = {{"user_id", ""}, {"total", "100"}};
        char v[16]; snprintf(v, sizeof(v), "%d", i);
        pairs[0][1] = v;
        insert_doc("ujorders", pairs, 2);
    }

    HugoResult r_opt, r_leg;
    exec_query("funden ujusers $rasoat ujorders local_field id target_field user_id", &r_opt);
    exec_legacy("funden ujusers $rasoat ujorders local_field id target_field user_id", &r_leg);
    result_free_disk(&r_opt);
    result_free_disk(&r_leg);
    db_teardown();
    PASS();
...
/* ===== Phase 10: Advanced Rules Tests ===== */

static void test_advanced_constant_fold_true(void) {
    TEST("advanced: constant_fold eliminates always-true filter");
    Arena *a = arena_new();

    /* Build: Filter(NULL predicate = always true) → Scan */
    LogicalPlan *scan = (LogicalPlan*)arena_alloc(a, sizeof(LogicalPlan));
    memset(scan, 0, sizeof(LogicalPlan));
    scan->type = LOP_SCAN;
    scan->estimated_rows = 100;

    LogicalPlan *filt = (LogicalPlan*)arena_alloc(a, sizeof(LogicalPlan));
    memset(filt, 0, sizeof(LogicalPlan));
    filt->type = LOP_FILTER;
    filt->filter.predicate = NULL; /* NULL = always true */
    filt->left = scan;
    filt->estimated_rows = 100;

    LogicalPlan *result = rule_constant_fold(filt, a);
    /* NULL predicate filter should be eliminated */
    ASSERT(result != NULL, "result is null");
    ASSERT(result->type == LOP_SCAN, "filter with NULL pred should be eliminated");

    arena_free(a);
    PASS();
}

static void test_advanced_pred_helpers(void) {
    TEST("advanced: pred_is_always_true/false work correctly");
    /* NULL condition = always true */
    ASSERT(pred_is_always_true(NULL) == 1, "NULL pred should be always true");
    ASSERT(pred_is_always_false(NULL) == 0, "NULL pred should not be always false");
    PASS();
}

static void test_advanced_rules_no_crash(void) {
    TEST("advanced: apply_advanced_rules doesn't crash on typical plan");
    Arena *a = arena_new();
    TokenList tl;
    hugo_tokenize("funden users haar age $bg 25 orange bi score desc lime 10", &tl);
    Query q; hugo_parse(&tl, &q);
    LogicalPlan *p = build_logical_plan(&q, a);
    ASSERT(p != NULL, "plan is null");

    /* Apply advanced rules — should not crash */
    LogicalPlan *after = apply_advanced_rules(p, a);
    ASSERT(after != NULL, "advanced rules returned null");
...
static void test_advanced_split_predicates(void) {
    TEST("advanced: rule_split_predicates pass-through for simple query");
    Arena *a = arena_new();
    TokenList tl;
    hugo_tokenize("funden users haar age $bg 25", &tl);
    Query q; hugo_parse(&tl, &q);
    LogicalPlan *p = build_logical_plan(&q, a);
    ASSERT(p != NULL, "plan is null");

    LogicalPlan *after = rule_split_predicates(p, a);
    /* Simple query has no AND — should pass through unchanged */
    ASSERT(after != NULL, "split_predicates returned null");
...
int main(void) {
    printf("========================================\n");
    printf(" Hugo DB — Stage 3: Optimizer Tests\n");
    printf("========================================\n\n");
    ...
    printf("\nPhase 7: Join Order DP\n");
    test_join_order_single_table();
    test_join_order_two_tables();
    test_join_order_enumerate_splits();
    test_join_order_e2e_same_result();

    printf("\nPhase 10: Advanced Rules\n");
    test_advanced_constant_fold_true();
    test_advanced_pred_helpers();
    test_advanced_rules_no_crash();
    test_advanced_split_predicates();
